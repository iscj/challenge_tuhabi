...
class PropertyModel(Connection):
	def filters(self, filters):
		...
		query_s = f"""SELECT p.address, p.city, s.name as status, p.price, p.description
										...
										WHERE sh.update_date IN(SELECT MAX(update_date) FROM habi_db.status_history GROUP BY property_id) AND (status_id=3 OR status_id=4 OR status_id=5) AND {clause_sql}"""
	...
	def format_filters(self, filters):
		...
		filters = [f'{filter_valids.get(_filter)}=("{value}")'.replace(" ", "") for _filter, value in filters.items() if _filter in filter_valids.keys() and value != None]
